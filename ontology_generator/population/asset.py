...
def process_asset_hierarchy(row: Dict[str, Any], 
    ...
    if area_ind:
        ...
        if property_mappings and "Area" in property_mappings:
            ...
            if area_physical_name: 
                context.set_prop(area_ind, "areaPhysicalCategoryName", area_physical_name)  # non-functional
            area_cat_code = safe_cast(row.get('GH_CATEGORY'), str)
            if area_cat_code: 
                context.set_prop(area_ind, "areaCategoryCode", area_cat_code)  # functional
...
def process_material(row: Dict[str, Any], 
    ...
    if property_mappings and "Material" in property_mappings:
        ...
        apply_property_mappings(mat_ind, material_mappings, row, context, "Material")
    else:
        # Fallback to hardcoded property assignments
        ...
        if mat_desc: 
            context.set_prop(mat_ind, "materialDescription", mat_desc)
...
def process_production_request(row: Dict[str, Any], 
    ...
    if property_mappings and "ProductionRequest" in property_mappings:
        ...
        apply_property_mappings(req_ind, request_mappings, row, context, "ProductionRequest")
    else:
        # Fallback to hardcoded property assignments
        ...
        if req_desc: 
            context.set_prop(req_ind, "requestDescription", req_desc)
